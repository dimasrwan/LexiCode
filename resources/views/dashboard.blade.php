...
<body class="bg-black text-zinc-100" x-data="{ openModal: false }">
    <nav class="border-b border-yellow-500/20 py-4 px-6 bg-zinc-950/50 backdrop-blur-md sticky top-0 z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="flex items-center gap-8">
                ...
                <button @click="openModal = true" class="bg-yellow-500 text-black px-4 py-1.5 rounded text-[10px] font-black uppercase hover:bg-white transition-all">
                    + New Project
                </button>
            </div>
        </div>
    ...
    <main class="max-w-7xl mx-auto py-12 px-6">
        <div class="mb-12">
            <h1 class="text-4xl font-extrabold text-white mb-2">PROJECTS<span class="text-yellow-500">_</span></h1>
            <p class="text-zinc-500 text-sm">Welcome back, Admin. System is operational.</p>
            ...
            <div x-data="{ show: true }" 
                ...
                class="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/50 text-yellow-500 text-xs font-bold uppercase tracking-[0.2em] flex items-center justify-between">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="project-card group relative bg-zinc-900/40 border border-zinc-800 p-6 rounded-xl hover:border-yellow-500/50 transition-all duration-300">
                ...
                <div class="flex justify-between items-center pt-4 border-t border-zinc-800">
                    <div class="text-[11px] text-zinc-500">
                        <span class="text-yellow-500 font-bold">{{ $project->modules_count }}</span> MODULES LOADED
                    </div>
            ...
            <div class="col-span-full border-2 border-dashed border-zinc-800 p-20 text-center rounded-2xl">
                <p class="text-zinc-600 italic">No projects found. Try running: <code class="text-yellow-600">php artisan db:seed</code></p>
            </div>
            @endforelse
        </div>
    </main>
        ...
        <div @click.away="openModal = false" class="bg-zinc-950 border border-yellow-500/30 w-full max-w-lg p-8 rounded-2xl shadow-2xl">
            ...
            <form action="{{ route('projects.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" required class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-yellow-500 text-white transition-colors" placeholder="e.g. LexiCode Mobile">
                </div>

                <div>
                    <label class="block text-[10px] font-black text-zinc-500 uppercase mb-2">Tech Stack</label>
                    <input type="text" name="tech_stack" required class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-yellow-500 text-white transition-colors" placeholder="e.g. FLUTTER, FIREBASE">
                </div>
